def qssl01 = ({
        ...
        def getFlowToMemcpy(contSt: ControlStructure): Boolean = {
          ...
          def flowFromAssignmentBeforeCondToMemcpy: Boolean = {
            ...
            def condReachable = arg.reachableByFlows(source.argument(2).ast.isIdentifier)
            if (condReachable.isEmpty) return false
            else if (!(condReachable.head.elements.size < 4)) return false
